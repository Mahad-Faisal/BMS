                            ...
                            bms_nn_output_t *nn_out)
{
    ...
    c7 = DWT_READ();
    uint32_t c_total1 = DWT_READ();

    g_bms_timing.phys_pred_us = cyc_to_us(c1 - c0);
    g_bms_timing.nn_block_us  = cyc_to_us(c3 - c2);
    g_bms_timing.kf_update_us = cyc_to_us(c5 - c4);
    g_bms_timing.thermal_us   = cyc_to_us(c7 - c6);
    g_bms_timing.total_us     = cyc_to_us(c_total1 - c_total0);
#endif
}
