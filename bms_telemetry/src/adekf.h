...
typedef struct
{
    uint32_t phys_pred_us;   /* LUT + state prediction + base V prediction */
    uint32_t nn_block_us;    /* bms_nn_residual_step: feature + NN + gate */
    uint32_t kf_update_us;   /* adaptive R + R0 + state/covariance update */
    uint32_t thermal_us;     /* thermal observer update */
    uint32_t total_us;       /* full adekf_step_core compute */
} bms_timing_breakdown_t;

extern volatile bms_timing_breakdown_t g_bms_timing;
