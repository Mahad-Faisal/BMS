...
/* ── Initial conditions ──────────────────────────────────────────────────── */
#define EST_SOC_INIT    0.9f
#define EST_R0_INIT     0.023f

#include "FreeRTOS.h"
#include "task.h"

#define EST_STACK_WORDS 8192U
#define EST_STACK_ALLOC_B ((uint32_t)(EST_STACK_WORDS * sizeof(StackType_t)))

static uint32_t g_est_stack_alloc_B = 0;
static uint32_t g_est_stack_free_w  = 0;
static uint32_t g_est_stack_free_B  = 0;
static uint32_t g_est_stack_used_B  = 0;

volatile bms_timing_breakdown_t g_bms_timing = {0};
/*
...
void estimator_task_entry(void *pvParameters)
{
    ...
    g_est_stack_alloc_B = EST_STACK_ALLOC_B;
    ...
    uart_send("t_ms,V_mV,I_mA,Tsurf_cC,"
              ...
              "stack_free_w,stack_free_B,stack_used_B,stack_alloc_B,"
              "t_phys_pred_us,t_nn_block_us,t_kf_update_us,t_thermal_us,t_step_total_us\r\n");
    ...
    for (;;)
    {
        ...
        g_est_stack_free_w = (uint32_t)free_words;
        g_est_stack_free_B = (uint32_t)(free_words * sizeof(StackType_t));
        g_est_stack_alloc_B = EST_STACK_ALLOC_B;

        if (g_est_stack_alloc_B >= g_est_stack_free_B)
        {
            g_est_stack_used_B = g_est_stack_alloc_B - g_est_stack_free_B;
        }
        else
        {
            g_est_stack_used_B = 0U;
        }
        ...
        if (got_mailbox && meas.fresh)
        {
            ...
            snprintf(line, sizeof(line),
                     ...
                     (unsigned long)g_est_stack_free_w,
                     (unsigned long)g_est_stack_free_B,
                     (unsigned long)g_est_stack_used_B,
                     (unsigned long)g_est_stack_alloc_B,
                     (unsigned long)g_bms_timing.phys_pred_us,
                     (unsigned long)g_bms_timing.nn_block_us,
                     (unsigned long)g_bms_timing.kf_update_us,
                     (unsigned long)g_bms_timing.thermal_us,
                     (unsigned long)g_bms_timing.total_us);
            uart_send(line);
        }
        t_ms += EST_PERIOD_MS;
