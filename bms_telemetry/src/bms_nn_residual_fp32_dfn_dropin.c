 ...
 *
 * FP32 runtime backend for Hybrid Gate065-DFN.
 *
 * Use this backend with:
 *   bms_nn_residual_dfn_aged_weights_fp32.h
 *
 * Important differences from the old v9 FP32 backend:
 *   - includes the DFN-aged weights header
 *   - uses HardTanh, not tanhf()
 *   - implements the final HardTanh output clamp
 *   - implements exact 600-sample rolling I_bias_60s used by v12 training
 */

#include "bms_nn_residual_fp32_dfn_dropin.h"
#include "bms_nn_residual_dfn_aged_weights_fp32.h"
...
/*
 * Gate065 non-amplification overlay:
 * correction must match raw-innovation sign and cannot exceed 65% of
 * |raw innovation|.
 */
...
static float bms_nn_hardtanh(float x)
{
    return bms_nn_clampf(x, BMS_NN_ACT_MIN, BMS_NN_ACT_MAX);
}
...
static void bms_nn_update_i_bias_rolling(bms_nn_ctx_t *ctx, float i_now_a)
{
    if (ctx->i_bias_count < BMS_NN_I_BIAS_WIN_LEN)
    {
        ctx->i_bias_hist[ctx->i_bias_head] = i_now_a;
        ctx->i_bias_sum += i_now_a;
        ctx->i_bias_count++;
    }
    else
    {
        float old = ctx->i_bias_hist[ctx->i_bias_head];
        ctx->i_bias_hist[ctx->i_bias_head] = i_now_a;
        ctx->i_bias_sum += i_now_a - old;
    }

    ctx->i_bias_head++;
    if (ctx->i_bias_head >= BMS_NN_I_BIAS_WIN_LEN)
    {
        ctx->i_bias_head = 0U;
    }

    if (ctx->i_bias_count > 0U)
    {
        ctx->i_bias_60s = ctx->i_bias_sum / (float)ctx->i_bias_count;
    }
    else
    {
        ctx->i_bias_60s = i_now_a;
    }
}
...
static float bms_nn_forward_norm(const float x_in[BMS_NN_N_IN])
{
    ...
    /*
     * Final HardTanh is part of the trained model:
     * Linear -> HardTanh -> Linear -> HardTanh -> Linear -> HardTanh.
     */
    return BMS_NN_V_CLAMP_V * bms_nn_hardtanh(acc);
}
                                     ...
                                     uint32_t plant_step)
{
    ...
    /*
     * Match pandas rolling(600, min_periods=1).mean() used in DFN v12 training:
     * the current sample is included in I_bias_60s.
     */
    bms_nn_update_i_bias_rolling(ctx, i_pack_a);
